...
from localio import summarize, csv_reader2, make_stats, abcd_master
...
def matrixDemoTestWorker(trials=None, tree_type=None, spill_rate=None, k_neighbors=None, tree_depth=None, files=None):
    run = 0
    ...
    for runs in range(trials):
        ...
        for k in range(len(validationGroups)):
            ...
            for test_point in range(len(validationGroups[k])):
                ...
                predicted = round(sum([1 if testing[kl][-1] > 0 else 0 for kl in knn_approx])/k_near)
...
def matrixTestMaster(trials, size=None, dimensions=None, tree_type=None, spill_rate=None, samp=None,
    ...
    nearest_neighbors = [1, 5, 10, 30, 100]
    ...
    sys.exit(0)
    """
    errorLog = open("C:\\Users\\Andrew\\PycharmProjects\\spatialtree\\datafiles\\errors.txt", 'w')
